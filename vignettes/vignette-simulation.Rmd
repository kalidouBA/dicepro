...
ggplot(df, aes(
  x = cor,
  y = reorder(cell_type, cor)
)) +
  geom_segment(aes(x = 0, xend = cor, yend = cell_type),
               color = "grey70") +
  geom_point(aes(color = cor > 0.7), size = 2.5) +
  scale_color_manual(values = c("#d7191c", "#2c7bb6"), guide = "none") +
  geom_vline(xintercept = 0.7, linetype = "dashed", color = "gray40") +
  xlim(-0.2, 1) +
  labs(x = "Pearson r", y = NULL,
       title = "Per-cell-type correlation (synthetic)") +
  theme_minimal()
```
...
| Compartment | Cell types (n) | Default alpha |
|---|---|---|
...
Visualizing the compartment-level totals confirms the expected dominance of
...
benchmark in a biologically grounded reference, making results easier to interpret in the context of real human tissue data.
