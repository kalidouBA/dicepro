...
**dicepro** addresses this limitation through an iterative joint optimization
that simultaneously:

- estimates cell-type proportions for *known* populations (supervised step,
  via CIBERSORTx ("CSx"), CIBERSORT ("CS"), FARDEEP, DCQ).
- discovers and quantifies *unknown* populations using Non-Negative Matrix
  Factorization (NMF) with L-BFGS-B optimization (unsupervised step).

Hyper-parameters $(\lambda, \gamma, p')$ controlling the NMF regularization
...
- **Method-agnostic supervised step** — plug in any supported deconvolution
  backend.
- **Automated hyper-parameter search** — random search over a log-uniform grid
...
- **Rich diagnostics** — Pareto plot and hyper-parameter
...
Request a token at <https://cibersortx.stanford.edu/> (you will first need to register).
...
A 13299 genes x 34 cell-type reference signature matrix derived from sorted
bulk RNA-seq profiles spanning five major tissue compartments:
Immune (9), Stromal (8), Endothelial (3), Epithelial (5), and Muscle (9).
Immune cells (e.g., B cells, T cells, monocytes, macrophages, NK cells)
Stromal cells (e.g., fibroblasts, MSC-like cells)
Endothelial cells
Epithelial cells
Muscle-related cells (e.g., smooth muscle, myocytes)
...
A 31422 genes x 12 bulk RNA-seq matrix of experimentally constructed cell
